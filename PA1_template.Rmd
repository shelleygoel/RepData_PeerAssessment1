...
The mean and median of total number of steps taken per day are 
`r as.integer(new_mean_steps)` and `r new_median_steps` respectively. These values differ from 
...
```
```{r steps_per_interval}
intervals<- as.factor(complete_activity$interval)

pdat <- data.frame(rep(intervals,2))
 
pdat$day <- rep("weekend",length(intervals))

pdat$steps <-tapply(complete_activity$steps[complete_activity$day=="weekend"],
                        intervals[complete_activity$day=="weekend"],
                        mean)

pdat$day[length(intervals)/2:] <- "weekday"
pdat$steps <- tapply(complete_activity$steps[complete_activity$day=="weekday"],
